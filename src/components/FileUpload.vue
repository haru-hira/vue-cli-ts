  ...
  methods: {
    ...
    submitUpload() {
      ...
      if (file.size < partSize) {
        ...
        axios
          .get('http://localhost:80/document/init-upload')
          .then(res => {
            axios
              .put(res.data.s3PresignedURL, file, {
                headers: {
              ...
              .then(() => {
                axios
                  .put('http://localhost:80/document/complete-upload/' + res.data.id, {
                    isSuccess: true,
                    fileName: file.name,
        ...
        FileType.fromBlob(file)
          ...
          .then(async res => {
            ...
            for (let rangeStart = 0; rangeStart < allSize; rangeStart += partSize) {
              ...
              await axios
                .post('http://localhost:80/document/split-upload', uploadParams, {
                  headers: {
            ...
            return axios.put('http://localhost:80/document/complete-split-upload/' + id, {
              isSuccess: true,
              uploadId: uploadId,
              key: key,
    ...
    download(): void {
      ...
      let contentType = '';
      axios
        .get('http://localhost:80/document/' + id)
        .then(res1 => {
